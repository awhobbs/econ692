...
\begin{frame}{Three Fundamental Structures}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.32\textwidth}
      ...
      \begin{tikzpicture}[>=stealth, scale=0.75]
        \node[draw, circle, minimum size=0.7cm, font=\small] (X) at (0, 0) {$X$};
        \node[draw, circle, minimum size=0.7cm, font=\small] (M) at (1.6, 0) {$M$};
        \node[draw, circle, minimum size=0.7cm, font=\small] (Y) at (3.2, 0) {$Y$};
        \draw[->] (X) -- (M);
        \draw[->] (M) -- (Y);
      \end{tikzpicture}\\[6pt]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{tikzpicture}[>=stealth, scale=0.9]
        ...
        \draw[->, usfgreen, line width=1.5pt] (PFL) -- (EMP)
          node[midway, below, font=\footnotesize, usfgreen] {$\tau = ?$};
      \end{tikzpicture}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[>=stealth, scale=0.88]
        ...
        % Chain path (mediator)
        \draw[->, usfgreen] (PFL) -- (CHILD);
        \draw[->, usfgreen] (CHILD) -- (EMP);
        % Direct path
        \draw[->, usfgreen, dashed]
          (PFL) to[bend right=55]
  ...
  \begin{columns}[T]
    \begin{column}{0.57\textwidth}
      ...
      \begin{tikzpicture}[>=stealth, scale=0.88]
        ...
        % Treatment effect
        \draw[->, usfgreen, line width=1.5pt] (PFL) -- (EMP)
          node[midway, above, font=\small, usfgreen] {$\tau$};
        % Confounders
        \draw[->] (POL) -- (PFL);
        \draw[->] (POL) -- (EMP);
        % Mediator
